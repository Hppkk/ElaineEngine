...
namespace Elaine
{
    ...
    RHISampler* VulkanRHI::getOrCreateDefaultSampler(RHIDefaultSamplerType type)
    {
        switch (type)
        {
        case Elaine::Default_Sampler_Linear:
        ...
        case Elaine::Default_Sampler_Nearest:
    ...
    RHISampler* VulkanRHI::getOrCreateMipmapSampler(uint32_t width, uint32_t height)
    {
        ...
        RHISampler* sampler;
        uint32_t  mip_levels = (uint32_t)floor(log2(std::max(width, height))) + 1;
    ...
    bool VulkanRHI::createGraphicsPipelines(RHIPipelineCache* pipelineCache, uint32_t createInfoCount, const RHIGraphicsPipelineCreateInfo* pCreateInfo, RHIPipeline*& pPipelines)
    {
        ...
        for (int i = 0; i < pipeline_shader_stage_create_info_size; ++i)
        {
            ...
            if (rhi_pipeline_shader_stage_create_info_element.pSpecializationInfo != nullptr)
            {
                ...
                for (uint32_t i = 0; i < rhi_pipeline_shader_stage_create_info_element.pSpecializationInfo->mapEntryCount; ++i)
                {
    ...
    bool VulkanRHI::waitForFencesPFN(uint32_t fenceCount, RHIFence* const* pFence, RHIBool32 waitAll, uint64_t timeout)
    {
        ...
        {
            const auto& rhi_fence_element = pFence[i];
    ...
    {
        vkDestroyShaderModule(m_device, ((VulkanShader*)shader)->getResource(), nullptr);
    ...
    void VulkanRHI::createInstance()
    {
        ...
        {
            LOG_ERROR("validation layers is not support!");
        }
        ...
        appInfo.pApplicationName = "Elaine Render";
        appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
        appInfo.pEngineName = "Elaine";
    ...
    {
        return m_enable_point_light_shadow;
    }
    ...
    SwapChainSupportDetails VulkanRHI::querySwapChainSupport(VkPhysicalDevice physical_device)
    {
        ...
        if (format_count != 0)
        {
            ...
            vkGetPhysicalDeviceSurfaceFormatsKHR(
                physical_device, m_surface, &format_count, details_result.formats.data());
        }
        ...
        if (presentmode_count != 0)
        {
            ...
            vkGetPhysicalDeviceSurfacePresentModesKHR(
                physical_device, m_surface, &presentmode_count, details_result.presentModes.data());
        }
